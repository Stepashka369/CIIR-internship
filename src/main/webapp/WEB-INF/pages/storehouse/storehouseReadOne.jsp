...
<html lang="en">
    ...
    <body>
        ...
        <h2>Storehouse: <%= storehouse.getAddress() %></h2>
        <p>Square: <%= storehouse.getSquare() %></p>
        <h2>Products in storehouse:</h2>
        <% for(Map.Entry<Good, Integer> pair : storehouse.getGoods().entrySet()) { %>
        <p><%= "Product: " + pair.getKey().getName() + " Quantity: " + pair.getValue()%></p>
        <% } %>
        <a href="<%="storehouse-delete-servlet?storehouseId=" + storehouse.getId() %>">Delete storehouse</a><br/>
        <a href="<%="storehouse-update-servlet?storehouseId=" + storehouse.getId() %>">Update storehouse</a><br/>
